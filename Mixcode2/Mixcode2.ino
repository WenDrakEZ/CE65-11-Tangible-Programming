...
String prev_data = ""; // initialize previous data to an empty string
String current_data = ""; // initialize current data
   ...
   for (;;) {
    ...
    if (data.length() > 0) {
      ...
      Serial.println("Data = " + String(data));
       if (data == "if" && prev_data != "") {
        // current data = if  previous data ไม่ว่าง
        String combined_data = prev_data + " if";
        prev_data = combined_data; // update previous data
        Serial.println("Prev case 1 = " + String(prev_data));
    } else if (data != "if" && prev_data != "") {
        // current data != "if,"   previous dataไม่ว่าง
        Serial.println("Prev case 2 = " + String(prev_data));
        prev_data = ""; // reset previous data
    } else if (data != "if" && prev_data == "") {
        // current data != if   previous data ว่าง
        Serial.println("data case 3 = " + String(data));
    }
...
//void Move(){
//  
//  if (posi >= 0 || dosi >= 0 || posi <= 0 || dosi <=0 ){
//    TurnLeft();
//    Stop();
//    delay(500);
//    posi = 0;
